...
switch ($action)
{
	#we add cabinet to recomandate table
	case 'add':
				
				#check to see if available place for this cab
				$judet_id = $param3;
				$cab_type = $param4;
				$check_avilable = CheckAvailablePlace($judet_id,$cab_type);
				if($check_avilable)
				{
					$tpl->set_file('tpl_recomandata', 'recomandate/add_recomandata.tpl');
					$tpl->set_var('MAIN_TITLE', 'Add cabinet to Recomandate');
					$tpl->set_var('CABINET_ID', $param1);
					$tpl->set_var('USER_ID', $param2);
					$get_firm_info = GetFirme($param2,$param1);
					$tpl->set_var('CAB_TITLE',$get_firm_info[0]['nume_firma']);
					#set colors
					...
					$tpl->parse('MAIN', 'tpl_recomandata');
				}
				else
				{
					$_SESSION['admin_info_type']='error';
					$_SESSION['admin_info'] = "No more place for this cabinet! Sorry - add a coment and put it on a waiting list.";
	...
	case 'insert':
		#let's insert cabinet to recomandate
		$cabinet_id = $param1;
		$user_id = $param2;
		$get_firm_info = GetFirme($user_id,$cabinet_id);
		$judet_id = $get_firm_info[0]['judet'] ;
		$type_id= $get_firm_info[0]['category_id'];

		InsertToRecomandate($user_id,$cabinet_id,$_POST['comment'],$judet_id,$type_id,$_POST['perioada']);
		$_SESSION['admin_info_type']='info';
		$_SESSION['admin_info'] = "Operation sucessfuly done. Do not forgot to generate the PROFORM BILL!!.";
		header('location: '.$_SERVER['HTTP_REFERER']);
		exit;
	break;
   ...
   $display_errors .= "<li>".$array_obligatorii['nume_loc_sector'].".</li>";
	...
	case 'list':
		...
		if (count($cabinete) != '0')
		{
			...
			while (list($k, $v) = each($cabinete))
			{
				...
				$tpl->set_var('MODIFICA_IMAGINI',"index.php?page=cabinete.modificaimg.".$v['id'].".".$v['nume_firma'] );
	...
	case 'save_edit':
		...
		else
		{
			...
			{	if(strpos($ky,"caractere")!==FALSE)
					...
					{ 
						$query="Update cabinete set ".$ky."='".addslashes($val)."' WHERE id='".$id_firma."'";
						$db->query($query);
						if($ky=="judet")
	...
	//echo $param1." ".$param2." ".$param3." ".$param4;exit;
			...
			{
				$Subject = ucfirst($_POST['nume_fir'])." este activ pe DentistOnline.ro";	
	...
	case 'cauta':
		...
		#set blocks
		$tpl->set_block('tpl_cauta', 'list_messages', 'list_messages_block');
		$tpl->set_block('tpl_cauta', 'if_messages', 'if_messages_block');
		if($param1!="" )
		{
				...
				if (count($cabinete) !='0')
				{
					...
					while (list($k, $v) = each($cabinete))
					{
						...
						$bc = ($i % 2)+1;
						$tpl->set_var("BGCOLOR", 'row'.$bc);
						if(substr($v['submision_date'],0,10)==date("Y-m-d"))
						{	$tpl->set_var("BGCOLOR", "today_add");	}
						
						if(substr($v['expire_date'],0,10)==date("Y-m-d"))
						...
						$tpl->set_var('USER_ID',$v['user_id'] );
						$tpl->set_var('FIRMA_ID',$v['id'] );
						$tpl->set_var('PARENT_MODULE', GetCabinetParents($v['id']));
						$get_contract_id = GetContractForCabinet($v['id']);
						$get_proforma_id = GetproformaForCabinet($v['id']);
						$tpl->set_var('PROFORMA_ID', $get_proforma_id);
						$tpl->set_var('CONTRACT_ID',$get_contract_id );
						$tpl->set_var('PERIOADA', $v['perioada']);
						$tpl->set_var('TYPE',ucfirst(substr($v['type'],0,3)) );
						
						$tpl->set_var('EXPIRARE_CABINET',AdminTimeFormat($v['expire_date'],'Y') );
						$tpl->set_var('ADAUGARE_CABINET',AdminTimeFormat($v['submision_date'],'Y') );
	...
	case 'modificaimg':
		...
		$tpl->set_var('NUME_FIRMA', ucfirst($param2));
	...
	case 'rearenge':
		...
		ReorderImages($param2, $param3);
		RearrangeImages($param1);
		header ('location: '.$_SERVER['HTTP_REFERER']);
		exit;
	break;
}
