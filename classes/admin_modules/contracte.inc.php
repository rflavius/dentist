...
function InsertContract($get_info_proforma )
{
	global $db;
	global $conf;
	$expirare_contract = date("y-m-d", mktime(0, 0, 0, date("m"), date("d"), date("Y")+$get_info_proforma['perioada'] ));
 ...
 values('".$get_info_proforma['user_id']."','".$get_info_proforma['firma_id']."','".addslashes($get_info_proforma['nume_firma'])."','".$get_info_proforma['perioada']."','".$get_info_proforma['type']."','Y','".$get_info_proforma['cost']."','".date("y-m-d")."','".$expirare_contract."','Y','".$get_info_proforma['id']."')";
	$db->query($query);
